-- Full fixed ESP + Aimbot + Player AimAssist (UPDATED: bounding-box center aiming for players + adjustable vertical offset)
...
-- ensure Drawing API exists (typical exploit env)
...
-- cleanup previous run (re-exec safety)
...
local settings = {
	...
	aimSmoothnessPlayers = 0.90,                -- Players smoothness
	aimSmoothnessDrones = 0.85,                 -- Drones smoothness  
	aimSmoothnessClaymores = 0.85,                              -- 0 = snap, ~0.8 = smooth
	fovRadius = 40,                             -- pixels
	drawFOV = false,
	drawAimIndicator = true,
	aimTargetPriority = {"Player","Claymore","Drone"},   -- add "Player" here if you want players prioritized
	aimOnlyWhenVisible = true,
	-- NEW: adjust bounding-box aim for players (world Y offset in studs)
	aimPlayerYOffset = 2, -- positive = aim higher (upwards), negative = aim lower
	
	-- FPS OPTIMIZATIONS
	espMaxDistance = 500, -- Only show ESP within this distance
}
GLOBAL.settings = settings

-- applySettings (creates/hides global drawings)
...
local espboxes = {}           -- keyed by player.Name -> Drawing (Square)
local espboxoutlines = {}
local espnames = {}

local espdrones = {}          -- keyed by Model instance -> Drawing (Square)
local espdroneoutlines = {}

local espclaymores = {}       -- keyed by Model instance -> Drawing (Square)
local espclaymoreoutlines = {}

-- UTILS: find enemies/drones/claymores
...
-- UTILS: get bounding box on-screen; returns nil if nothing in front of camera
...
-- UTILS: model world position (PrimaryPart, Handle, or center)
...
-- ===== NEW: Bounding box center calculation =====
...
-- Use bounding-box center for aiming players; apply vertical offset setting
local function getBoundingBoxAimPosition(model)
	if not model then return nil end
	local bboxCenter = getBoundingBoxCenter(model)
	if bboxCenter then
		-- apply adjustable vertical offset (world units / studs)
		local yOffset = settings.aimPlayerYOffset or 0
		return bboxCenter + Vector3.new(0, yOffset, 0), bboxCenter
	end
	-- fallback
	return getModelWorldPosition(model), nil
end

-- DRAWING FACTORY (unchanged)
...
local function mainESP(model)
	...
	-- Distance check - skip if too far (FPS OPTIMIZATION)
	...
	if not ok or not boundsOrErr then
		-- graceful: hide any existing drawings
		if espboxes[nameKey] then safeSetVisible(espboxes[nameKey], false) end
		if espboxoutlines[nameKey] then safeSetVisible(espboxoutlines[nameKey], false) end
		if espnames[nameKey] then safeSetVisible(espnames[nameKey], false) end
		return
...
-- Drone/claymore helpers (unchanged)
...
local function drone_esp(model)
	...
	-- Distance check (FPS OPTIMIZATION)
	if settings.espMaxDistance > 0 then
...
-- ===== BOUNDING BOX AIM ASSIST PICKER =====
...
local function makeBlacklist()
	local list = {}
	if lp and lp.Character then table.insert(list, lp.Character) end
	for _, name in ipairs({"ViewModels", "Viewmodels", "viewmodels"}) do
		if Workspace:FindFirstChild(name) then
			table.insert(list, Workspace[name])
		end
	end
	return list
end

-- pickFromList now supports kinds: "Player" uses bounding box center (+offset); others fall back to getModelWorldPosition
local function pickFromList(list, kind)
	...
	for _, m in pairs(list) do
		if m and m:IsDescendantOf(Workspace) then
			local worldPos, bboxCenter
			if kind == "Player" then
				worldPos, bboxCenter = getBoundingBoxAimPosition(m)
			else
				worldPos = getModelWorldPosition(m)
			end
			if worldPos then
				local screenVec3, onScreen = camera:WorldToViewportPoint(worldPos)
				if onScreen and screenVec3.Z > 0 then
	...
	-- sort by closeness to crosshair
	table.sort(candidates, function(a,b) return a.dist < b.dist end)

	-- visibility checks (limit raycasts)
	local best = nil
	local bestDist = math.huge
	for i, c in ipairs(candidates) do
		if i > 6 then break end
		local isVisible = true
		if settings.aimOnlyWhenVisible then
			local rayOrigin = camera.CFrame.Position
			local rayDirection = (c.world - rayOrigin)
			sharedRayParams.FilterDescendantsInstances = makeBlacklist()
			local result = Workspace:Raycast(rayOrigin, rayDirection, sharedRayParams)
			if result and result.Instance then
				local hitModel = result.Instance:FindFirstAncestorOfClass("Model")
				if (result.Instance.Transparency < 1 and result.Instance.CanCollide)
				and (not hitModel or hitModel ~= c.model) then
					isVisible = false
				end
			end
		end
		if isVisible and c.dist < bestDist then
			bestDist = c.dist
			best = c
		end
	end
...
-- gather enemy players if Player priority requested
local enemiesList = {}
...
lp:GetPropertyChangedSignal("Team"):Connect(updateEnemyList)
RunService.Heartbeat:Connect(function()
	if tick() % 3 < 0.03 then updateEnemyList() end
end)

-- pickAimbotTarget uses pickFromList for each prioritized kind
...
-- MAIN LOOP + ESP throttling (unchanged core loop, but uses new picker)
local espAccumulator = 0
local espUpdateRate = 1/20 -- Reduced from 1/30 to 1/20 for better FPS
...
-- ensure drawings exist per settings
applySettings({})

-- throttle parameters (tweak if desired)
local scanInterval = 0.08 -- seconds between heavy target scans
local lastScanTime = 0
local lastTargetInfo = nil
local lastTargetKind = nil
...
RunService.RenderStepped:Connect(function(dt)
	...
	-- update fov circle center each frame
	...
	-- keep aiming toggle updated if using toggle mode
	if not settings.aimHoldToLock then aiming = aimToggleState end

	-- fetch entities once (light)
	local enemies, drones, claymores = getenemies()

	-- Heavy scan only every scanInterval seconds (reduces raycasts)
	if tick() >= lastScanTime + scanInterval then
		lastScanTime = tick()
		local ok, tinfo, tkind = pcall(function() return pickAimbotTarget(drones, claymores) end)
		if ok and tinfo then
			lastTargetInfo = tinfo
			lastTargetKind = tkind
		else
			lastTargetInfo = nil
			lastTargetKind = nil
		end
	...
	-- AIM: use lastTargetInfo for per-frame smoothing & indicator
	if settings.aimbotEnabled and lastTargetInfo then
		-- draw indicator if configured
		if settings.drawAimIndicator then
			local ind = getGlobalDrawing("aimIndicator")
			if ind then
				pcall(function()
					ind.Position = lastTargetInfo.screen
					ind.Visible = true
					if lastTargetKind == "Claymore" then ind.Color = Color3.fromRGB(255,140,0)
					elseif lastTargetKind == "Drone" then ind.Color = Color3.fromRGB(0,255,0)
					else ind.Color = Color3.fromRGB(255,255,255) end
				end)
			end
		end

		-- smoothing while aiming
		if aiming then
			local current = camera.CFrame
			local newCF = smoothLookAt(current, lastTargetInfo.world, lastTargetKind)
	...
	-- ESP: throttle heavy updates with batching (FPS OPTIMIZATION)
	espAccumulator = espAccumulator + dt
	if espAccumulator >= espUpdateRate then
		espAccumulator = 0

		-- Batch process: collect all entities first, then update drawings
		...
		-- Drones and Claymores (limit processing for FPS)
		local maxDrones = 8  -- Increased limit for drones
		local maxClaymores = 5 -- Increased limit for claymores
		...
		-- hide player drawings for invalid players
		...
		-- cleanup removed models (drones/claymores)
...
-- expose a helper to stop/cleanup
